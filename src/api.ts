...
export const offlineConfig:Config={transcription_enabled:false,transcription_mode:'disabled',rewrite_mode:'unavailable',processor:null,retention_notice:'Recording is local-only in this version. Audio stays in this browser until you download or discard it.',max_audio_seconds:90,max_audio_bytes:8*1024*1024,max_text_chars:4000};
export async function configuration(signal:AbortSignal):Promise<Config>{const r=await fetch('/v1/config',{signal,cache:'no-store'});if(!r.ok)throw new Error('unavailable');return {...offlineConfig,...await r.json(),retention_notice:offlineConfig.retention_notice};}
